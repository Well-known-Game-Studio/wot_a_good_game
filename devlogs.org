...
- [X] CommonUI widget doens't show for all keyboard input
...
- [X] Block out overworld (spine of the world, aiel waste, blight, ocean)
- [X] Figure out scale of overworld models
- [ ] Block out Emonds Field

** Code

- [X] Save player inventory to state so that it persists between level loads
- [ ] Store quest / level state when leaving so it is the same upon return
- [ ] Fix / improve geometry script tools so they perform better (wall is really bad)

** Game

- [ ] Have tiny models hurt when they touch you (e.g. melee actors)
- [ ] Add effects (butterflies and dust motes) to the scene
- [ ] add niagara smoke effects (chimney)
...
- [ ] Simplify world / foliage voxel models
- [ ] Simplify sword animation
- [ ] Make models for overworld scale
- [ ] rig dog
- [ ] rig raccoon
- [ ] rig duck
- [ ] Animate horse
...
- [X] Revamp attack / action and menu interactions
- [X] Add back parry support
- [X] Fix bugs in menu interaction and look/attack interaction
- [X] Add projectile shooter blueprints for arrows and magic projectiles
- [X] Improve dash action and allow it to have an effect
- [X] Improve particle effect for dash (custom effect)

* 2025-06

- [X] Add stone of tear, white tower, and dragonmount to the overworld
- [X] Use procedural content generation (PCG) to automatically place mountains and trees in the overworld
- [X] Fix game time transitions so that effects (fire, etc.) have right state
- [X] Refactor touch/input handling to allow touch to be fully playable / have all inputs / features
- [X] Remove need for unhighlight timers via refactoring highlighting system
- [X] Use overlay material to highlight individual foliage instances instead of all instances
- [X] Added blender tools for importing maps, making models, and exporting models
- [X] Update deprecated API usage to move to UE5.6
- [X] Add loading images and app icons
- [X] Allow touch / mouse attack to aim where the mouse is looking
- [X] Hide/show touch interface based on detected input
- [X] Configure android/ios/other platforms to have all controllers
- [X] Flesh out overworld a little bit
- [X] Add missing config for UI / icons for keyboad + mouse
- [X] Fix bug when opening menu while jumping
- [X] Hide chest until quest complete

* 2025-05

- [X] FIX: Sprint & inventory UI don't interact well
- [X] FIX: NavMesh not properly taking into account the foliage: https://forums.unrealengine.com/t/foliage-not-affecting-nav-mesh-in-ue5-works-in-4-26/532988
- [X] Update to enhanced input system
- [X] Make ladder
- [X] Make fence / pen for holding animals (script / spline)
- [X] Allow new character to climb ladder
- [X] Start stubbing out dialogue system
- [X] highlight interactable objects & foliage
- [X] add niagara water effects
- [X] add niagara smoke effects (campfire)
- [X] Add particle effects to waterfall (top & bottom)
- [X] mouse + keyboard support (to aim where mouse looks)
- [X] Update Arrow Projectile to be Physical projectile working for spears and such as well
- [X] Allow default items to be randomized on instance spawn
- [X] Compilation and deployment for Android + iOS
