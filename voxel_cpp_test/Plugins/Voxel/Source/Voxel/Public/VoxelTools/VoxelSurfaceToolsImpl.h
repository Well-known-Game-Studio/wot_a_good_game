...
public:
	...
	static void GetStrengthMaskBasisImpl(
		FVector PlaneNormal,
		...
		FVector& OutPlaneY)
	{
		...
		const FVector PlaneY = FVector::CrossProduct(PlaneNormal, PlaneTangent);
		PlaneTangent = FVector::CrossProduct(PlaneY, PlaneNormal);

		OutPlaneX = PlaneTangent;
		OutPlaneY = PlaneY;
	}
	static void ApplyStrengthMaskImpl(
		const FVoxelSurfaceEditsVoxelsInfo& Info,
		TArray<FVoxelSurfaceEditsVoxel>& Voxels,
		const TVoxelTexture<float>& Mask,
		const FVoxelVector& EditPosition,
		const float ScaleX,
		const float ScaleY,
		const FVector& PlaneX,
		const FVector& PlaneY,
		const EVoxelSamplerMode SamplerMode)
	{
		VOXEL_TOOL_FUNCTION_COUNTER(Voxels.Num());
		const float HalfSizeX = Mask.GetSizeX() / 2;
		const float HalfSizeY = Mask.GetSizeY() / 2;

		for (auto& Voxel : Voxels)
		{
			const FVector Position = (Voxel.Position - EditPosition).ToFloat();
			const float X = FVector::DotProduct(Position, PlaneX) / ScaleX;
			const float Y = FVector::DotProduct(Position, PlaneY) / ScaleY;

			Voxel.Strength *= Mask.Sample<float>(HalfSizeX + X, HalfSizeY + Y, SamplerMode);
		}
	}
