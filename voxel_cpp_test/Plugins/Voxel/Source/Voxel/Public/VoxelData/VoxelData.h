...
public:
	...
	/**
	 * Get diff arrays to allow network transmission. No lock required
	 ...
	 */
	void GetDiffs(TArray<TVoxelChunkDiff<FVoxelValue>>& OutValueDiffQueue, TArray<TVoxelChunkDiff<FVoxelMaterial>>& OutMaterialDiffQueue);

	/**
	 * Load values and materials from diffs, and add modified positions. No lock required
	 ...
	 */
	void LoadFromDiffs(TArray<TVoxelChunkDiff<FVoxelValue>>& ValueDiffQueue, TArray<TVoxelChunkDiff<FVoxelMaterial>>& MaterialDiffQueue, TArray<FVoxelIntBox>& OutModifiedBounds);
