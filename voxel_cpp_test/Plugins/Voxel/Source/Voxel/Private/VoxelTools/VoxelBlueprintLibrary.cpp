...
#include "VoxelSpawners/IVoxelSpawnerManager.h"
	...
	EVoxelSpawnerActorSpawnType SpawnType)
{
	...
	World->GetInstancedMeshManager().SpawnActorsInArea(Bounds, World->GetData(), SpawnType, OutActors);
}
	...
	const FVector FloatingDetectionOffset)
{
	...
	if (!Mesh)
	{
		FVoxelMessages::Error("AddInstances: Mesh is null!");
		return;
	}
	...
	FVoxelSpawnerTransforms VoxelSpawnerTransforms;
	VoxelSpawnerTransforms.TransformsOffset = World->GetInstancedMeshManager().ComputeTransformsOffset(Bounds);
	VoxelSpawnerTransforms.Matrices.Reserve(Transforms.Num());
	
	for (int32 Index = 0; Index < Transforms.Num(); Index++)
	{
		FTransform Transform = Transforms[Index];
		Transform.AddToTranslation(-World->VoxelSize * FVector(VoxelSpawnerTransforms.TransformsOffset + World->GetWorldOffset()));
		
		FVoxelSpawnerMatrix Matrix(Transform.ToMatrixWithScale());
		if (Colors.IsValidIndex(Index))
		{
			const FVoxelMaterial Material = FVoxelMaterial::CreateFromColor(Colors[Index]);
			Matrix.SetRandomInstanceId(Material.GetPackedColor());
		}
		Matrix.SetPositionOffset(-FloatingDetectionOffset);
		VoxelSpawnerTransforms.Matrices.Add(Matrix);
	}

	FVoxelInstancedMeshAndActorSettings MeshAndActorSettings;
	MeshAndActorSettings.Mesh = Mesh;
	MeshAndActorSettings.MeshSettings = InstanceSettings;
	MeshAndActorSettings.ActorSettings = ActorSettings;

	World->GetInstancedMeshManager().AddInstances(MeshAndActorSettings, VoxelSpawnerTransforms, Bounds);
}
...
void UVoxelBlueprintLibrary::RegenerateSpawners(AVoxelWorld* World, FVoxelIntBox Bounds)
{
	...
	World->GetSpawnerManager().Regenerate(Bounds);
}
...
void UVoxelBlueprintLibrary::MarkSpawnersDirty(AVoxelWorld* World, FVoxelIntBox Bounds)
{
	...
	World->GetSpawnerManager().MarkDirty(Bounds);
}
...
FVoxelSpawnersSave UVoxelBlueprintLibrary::GetSpawnersSave(AVoxelWorld* World)
{
	...
	World->GetSpawnerManager().SaveTo(Save.NewMutable());
	return Save;
}
...
void UVoxelBlueprintLibrary::LoadFromSpawnersSave(AVoxelWorld* World, const FVoxelSpawnersSave& Save)
{
	...
	World->GetSpawnerManager().LoadFrom(Save.Const());
}
...
{
	return World && World->IsCreated() && World->GetSpawnerManager().GetTaskCount() > 0;
}
