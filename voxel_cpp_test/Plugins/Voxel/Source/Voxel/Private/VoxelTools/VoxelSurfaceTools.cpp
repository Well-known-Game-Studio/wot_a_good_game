	...
	bool bConvertToVoxelSpace)
{
	...
	const FVoxelVector EditPosition = GET_VOXEL_TOOL_REAL(InEditPosition);

	FVector OutPlaneX, OutPlaneY;
	FVoxelSurfaceToolsImpl::GetStrengthMaskBasisImpl(PlaneNormal, PlaneTangent, OutPlaneX, OutPlaneY);
	...
	return
	{
		...
		[=](auto& Info, auto& Voxels)
		{
			FVoxelSurfaceToolsImpl::ApplyStrengthMaskImpl(Info, Voxels,
				Mask.Texture,
				EditPosition,
				ScaleX,
				ScaleY,
...
void UVoxelSurfaceTools::GetStrengthMaskScale(
	...
	bool bConvertToVoxelSpace)
{
	CHECK_VOXELWORLD_FOR_CONVERT_TO_VOXEL_SPACE_VOID();

	SizeX = GET_VOXEL_TOOL_REAL(SizeX);
	SizeY = GET_VOXEL_TOOL_REAL(SizeY);

	ScaleX = SizeX / Mask.Texture.GetSizeX();
	ScaleY = SizeY / Mask.Texture.GetSizeY();
}
