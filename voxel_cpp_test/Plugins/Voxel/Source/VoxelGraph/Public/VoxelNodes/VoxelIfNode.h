...
class FVoxelIfComputeNode : public TVoxelExecComputeNodeHelper<EVoxelComputeNodeExecType::If>
{
public:
	const EVoxelNodeIfBranchToUseForRangeAnalysis BranchToUseForRangeAnalysis;

	FVoxelIfComputeNode(const UVoxelNode_If& Node, const FVoxelCompilationNode& CompilationNode)
		: TVoxelExecComputeNodeHelper(Node, CompilationNode)
		, BranchToUseForRangeAnalysis(Node.BranchToUseForRangeAnalysis)
	{
	}
};
