...
TVoxelSharedPtr<FVoxelComputeNode> GetComputeNode(const FVoxelCompilationNode& InCompilationNode) const override; public:

#define GENERATED_VOXELNODE_IMPL_IMPL(Name, ...) \
TVoxelSharedPtr<FVoxelComputeNode> Name::GetComputeNode(const FVoxelCompilationNode& InCompilationNode) const \
{ \
	__VA_ARGS__ \
	return MakeShareable(new FLocalVoxelComputeNode(*this, InCompilationNode)); \
}

#define GENERATED_DATA_COMPUTE_NODE_BODY_IMPL(FLocalVoxelComputeNode) \
virtual double ComputeStats(double InThreshold, bool bSimulateCpp, FVoxelNodeInputBuffer Inputs, const FVoxelContext& Context) const override final \
{ \
	return FVoxelNodeHelpers::ComputeStatsImpl(this, InThreshold, bSimulateCpp, Inputs, Context); \
} \
virtual double ComputeRangeStats(double InThreshold, bool bSimulateCpp, FVoxelNodeInputRangeBuffer Inputs, const FVoxelContextRange& Context) const override final \
{ \
	return FVoxelNodeHelpers::ComputeRangeStatsImpl(this, InThreshold, bSimulateCpp, Inputs, Context); \
} \
virtual void CacheFunctionPtr() override final \
{ \
	this->ComputeFunctionPtr = static_cast<FVoxelDataComputeNode::FComputeFunction>(&FLocalVoxelComputeNode::Compute); \
}

#define GENERATED_DATA_COMPUTE_NODE_BODY() GENERATED_DATA_COMPUTE_NODE_BODY_IMPL(FLocalVoxelComputeNode)

#define GENERATED_COMPUTE(InputsDef, OutputsDef, ...) \
void Compute(FVoxelNodeInputBuffer Inputs, FVoxelNodeOutputBuffer Outputs, const FVoxelContext& _C0) const override \
{ \
	InputsDef \
	OutputsDef \
	__VA_ARGS__ \
} \
void ComputeRange(FVoxelNodeInputRangeBuffer Inputs, FVoxelNodeOutputRangeBuffer Outputs, const FVoxelContextRange& _C0) const override \
{ \
	InputsDef \
	OutputsDef \
	__VA_ARGS__ \
...
#define GENERATED_VOXELNODE_IMPL(Name, InputsDef, OutputsDef, Code) GENERATED_VOXELNODE_IMPL_IMPL(Name, GENERATED_COMPUTENODE(InputsDef, OutputsDef, Code))
...
public: \
	...
	{ \
        if(InputCount == 2) \
		{ \
			Outputs[0].Get<Type>() = Op(Inputs[0].Get<Type>(), Inputs[1].Get<Type>()); \
		} \
		else \
		{ \
			auto X = Inputs[0].Get<Type>(); \
			for (int32 I = 1; I < InputCount; I++) \
			{ \
				X = Op(X, Inputs[I].Get<Type>()); \
			} \
			Outputs[0].Get<Type>() = X; \
		} \
	} \
	void ComputeRange(FVoxelNodeInputRangeBuffer Inputs, FVoxelNodeOutputRangeBuffer Outputs, const FVoxelContextRange& _C0) const override \
	{ \
	...
	void ComputeCpp(const TArray<FString>& Inputs, const TArray<FString>& Outputs, FVoxelCppConstructor& Constructor) const override \
	{ \
		Constructor.AddLine(FVoxelNodeHelpers::GetPrefixOpLoopString(Inputs, Outputs, InputCount, #Op)); \
	} \
	void ComputeRangeCpp(const TArray<FString>& Inputs, const TArray<FString>& Outputs, FVoxelCppConstructor& Constructor) const override \
	{ \
		Constructor.AddLine(FVoxelNodeHelpers::GetPrefixOpLoopString(Inputs, Outputs, InputCount, #Op)); \
...
#define GENERATED_VOXELNODE_IMPL_PREFIXOPLOOP(Name, Op, Type) GENERATED_VOXELNODE_IMPL_IMPL(Name, GENERATED_COMPUTENODE_PREFIXOPLOOP(Op, Type))
...
public: \
	...
	{ \
        if(InputCount == 2) \
		{ \
			Outputs[0].Get<Type>() = Inputs[0].Get<Type>() Op Inputs[1].Get<Type>(); \
		} \
		else \
		{ \
			auto X = Inputs[0].Get<Type>(); \
	...
	void ComputeCpp(const TArray<FString>& Inputs, const TArray<FString>& Outputs, FVoxelCppConstructor& Constructor) const override \
	{ \
		Constructor.AddLine(FVoxelNodeHelpers::GetInfixOpLoopString(Inputs, Outputs, InputCount, #Op)); \
	} \
	void ComputeRangeCpp(const TArray<FString>& Inputs, const TArray<FString>& Outputs, FVoxelCppConstructor& Constructor) const override \
	{ \
		Constructor.AddLine(FVoxelNodeHelpers::GetInfixOpLoopString(Inputs, Outputs, InputCount, #Op)); \
