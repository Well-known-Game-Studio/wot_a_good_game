...
public:
	...
	void AddNodeToSelect(const FVoxelCompilationNode* Node);
...
namespace FEnsureVoxelGraphHelper
{
	// Returns true
	bool Check(FVoxelGraphErrorReporter& ErrorReporter, const FString& Expr, const FString& File, int32 Line, const FVoxelCompilationNode* Node);
}

#define ensureVoxelGraphImpl(Expr, Node, ErrorReporter) (ensure((Expr)) || FEnsureVoxelGraphHelper::Check(ErrorReporter, #Expr, __FILE__, __LINE__, Node))
#define ensureVoxelGraph(Expr, Node) ensureVoxelGraphImpl(Expr, Node, ErrorReporter)
#define ensureVoxelGraphExitPass(Expr, Node) if (!ensureVoxelGraphImpl(Expr, Node, Compiler.ErrorReporter)) return;
