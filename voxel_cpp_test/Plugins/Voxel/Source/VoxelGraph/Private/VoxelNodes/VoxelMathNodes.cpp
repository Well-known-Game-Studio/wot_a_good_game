...
TSharedPtr<FVoxelCompilationNode> UVoxelNode_SwitchInt::GetCompilationNode() const
{
	return MakeShared<FVoxelSwitchCompilationNode>(*this);
}
GENERATED_VOXELNODE_IMPL
(
...
TSharedPtr<FVoxelCompilationNode> UVoxelNode_SwitchFloat::GetCompilationNode() const
{
	return MakeShared<FVoxelSwitchCompilationNode>(*this);
}
GENERATED_VOXELNODE_IMPL
...
TSharedPtr<FVoxelCompilationNode> UVoxelNode_SwitchColor::GetCompilationNode() const
{
	return MakeShared<FVoxelSwitchCompilationNode>(*this);
}
GENERATED_VOXELNODE_IMPL
...
TVoxelSharedPtr<FVoxelComputeNode> UVoxelNode_NormalizeSum::GetComputeNode(const FVoxelCompilationNode& InCompilationNode) const
{
	...
	public:
		...
		FLocalVoxelComputeNode(const UVoxelNode_NormalizeSum& Node, const FVoxelCompilationNode& CompilationNode)
			: FVoxelDataComputeNode(Node, CompilationNode)
		{
			check(InputCount == OutputCount);
		}
		...
		void Compute(FVoxelNodeInputBuffer Inputs, FVoxelNodeOutputBuffer Outputs, const FVoxelContext& Context) const override
		{
			...
			{
				Sum += FMath::Max<v_flt>(0, Inputs[Index].Get<v_flt>());
			}
			if (Sum == 0) Sum = 1;
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Outputs[Index].Get<v_flt>() = FMath::Max<v_flt>(0, Inputs[Index].Get<v_flt>()) / Sum;
			}
		}
		...
		{
			TVoxelRange<v_flt> Sum = 0;
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Sum += FVoxelNodeFunctions::Max<v_flt>(0, Inputs[Index].Get<v_flt>());
			}
			if (Sum.IsSingleValue() && Sum.GetSingleValue() == 0) Sum = 1;
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Outputs[Index].Get<v_flt>() = TVoxelRange<v_flt>::Intersection({ 0, 1 }, Inputs[Index].Get<v_flt>() / Sum);
			}
		}
		void ComputeCpp(const TArray<FString>& Inputs, const TArray<FString>& Outputs, FVoxelCppConstructor& Constructor) const override
		{
			...
			Constructor.AddLine("v_flt Sum = 0;");
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Constructor.AddLinef(TEXT("Sum += FMath::Max<v_flt>(0, %s);"), *Inputs[Index]);
			}
			Constructor.AddLine("if (Sum == 0) Sum = 1;");
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Constructor.AddLinef(TEXT("%s = FMath::Max<v_flt>(0, %s) / Sum;"), *Outputs[Index], *Inputs[Index]);
			}
		...
		{
			Constructor.StartBlock();
			Constructor.AddLine("TVoxelRange<v_flt> Sum = 0;");
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Constructor.AddLinef(TEXT("Sum += FVoxelNodeFunctions::Max<v_flt>(0, %s);"), *Inputs[Index]);
			}
			Constructor.AddLine("if (Sum.IsSingleValue() && Sum.GetSingleValue() == 0) Sum = 1;");
			for (int32 Index = 0; Index < InputCount; Index++)
			{
				Constructor.AddLinef(TEXT("%s = TVoxelRange<v_flt>::Intersection({ 0, 1 }, %s / Sum);"), *Outputs[Index], *Inputs[Index]);
			}
			Constructor.EndBlock();
		}
	};
