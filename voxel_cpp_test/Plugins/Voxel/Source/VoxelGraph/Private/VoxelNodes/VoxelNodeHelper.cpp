...
TSharedPtr<FVoxelCompilationNode> UVoxelSetterNode::GetCompilationNode() const
{
	auto Result = MakeShared<FVoxelSetterCompilationNode>(*this);
	Result->OutputIndex = GetOutputIndex();
	return Result;
}

TSharedPtr<FVoxelCompilationNode> UVoxelPureNode::GetCompilationNode() const
{
	return MakeShared<FVoxelDefaultPureCompilationNode>(*this);
}

TSharedPtr<FVoxelCompilationNode> UVoxelNodeWithDependencies::GetCompilationNode() const
{
	auto Result = MakeShared<FVoxelAxisDependenciesCompilationNode>(*this);
	Result->DefaultDependencies = GetNodeDependencies();
	return Result;
}
